    ...
    internal class ModifierHolder
    {
        ...
        internal ModifierHolder(SubsystemTerrain terrainInput, TerrainChunkGeneratorProviderActive activeChunkProvider, float num, Random rand)
        {
            ...
            this.holder = new float[]
            {
                ...
                0.03f * (float)Math.Pow(2, 41), //TGTurbulenceFreq3
                16f, //TGTurbulenceOctaves4
                0.5f * 700, //TGTurbulencePersistence5
                55f, //TGDensityBias6
                ...
                0.32f - 0.31f, //this.TGHillsPercentage = 13
                0.15f - 0.14f, //this.TGMountainsPercentage = 14
        ...
        private void GenerateTerrain(TerrainChunk chunk, int x1, int z1, int x2, int z2)
        {
            ...
            //u = 15;
            int oceanLevel = u;
            for (int i = 0; i < grid3d.SizeX - 1; i++)
            {
                for (int j = 0; j < grid3d.SizeZ - 1; j++)
                {
                    for (int k = 0; k < grid3d.SizeY - 1; k++)
                    {
                        grid3d.Get8(i, k, j, out float r0, out float r1, out float r2, out float r3, out float r4, out float r5, out float r6, out float r7);
                        double d0 = (r1 - r0) / 4f;
                        double d1 = (r3 - r2) / 4f;
                        double d2 = (r5 - r4) / 4f;
                        double d3 = (r7 - r6) / 4f;
                        ...
                        for (int a = 0; a < 4; a++)//4
                        {
                            ...
                            for (int b = 0; b < 4; b++)
                            {
                                ...
                                //int x3 = x1 + a + i * 4;
                                //int z3 = z1 + b + j * 4;
                                float x4 = grid2d.Get(a + i * 4, b + j * 4);
                                float x5 = grid2d2.Get(a + i * 4, b + j * 4);
                                int temperatureFast = chunk.GetTemperatureFast(x1 + a + i * 4, z1 + b + j * 4);
                                int humidityFast = chunk.GetHumidityFast(x1 + a + i * 4, z1 + b + j * 4);
                                ...
                                for (int c = 0; c < 8; c++)
                                {
                                    ...
                                    chunk.SetCellValueFast(x1 + a + i * 4, c + k * 8, z1 + b + j * 4, blockID);
                                    d13 += d12;
                                }
                                d10 += d8;
                                d11 += d9;
